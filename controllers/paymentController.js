...
export const buySubscription = catchAsyncError(async (req, res, next) => {
    ...
    const plan_id = process.env.PLAN_ID || "plan_LtmutFvU3rXvVs"
...
export const paymentVerification = catchAsyncError(async (req, res, next) => {
    ...
    if (!isAuthentic) return res.redirect(`${procees.env.FRONEND_URL}/paymentfail`)
    await Payment.create({ razorpay_signature, razorpay_payment_id, razorpay_subscription_id })
    user.subscription.status = "active"
    await user.save()
    res.redirect(`${procees.env.FRONEND_URL}/paymentsuccess?reference=${razorpay_payment_id}`)
})
...
export const cancelSubscription = catchAsyncError(async (req, res, next) => {
    ...
    await user.save()
    res.status(200).json({ success: true, message: refund ? "Subscription Cancelled, You will receive full refund within 7 days." : "Subscription Cancelled, Now Refund initiated as subscription was cancelled after 7 days." })
})